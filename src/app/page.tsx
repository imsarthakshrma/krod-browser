  ...
  return (
    <div className={cn("h-screen flex flex-col md:flex-row", darkMode && "dark")}>
      ...
      >
        ...
        <div className="h-16 flex items-center justify-between px-4">
          {(!sidebarCollapsed || (isMobileView && showMobileSidebar)) && (
            <div className="flex items-center gap-2">
              <h2 className="font-semibold text-xl text-neutral-100">Krod AI</h2>
            </div>
          )}
          <Button
            variant="ghost"
            ...
            className="w-8 h-8 p-0 text-slate-500 hover:text-foreground hover:bg-slate-100 dark:hover:bg-slate-800"
          >
        ...
        <div className="p-2">
          <Button
            ...
            className={cn(
              "w-full justify-start gap-2 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700",
        ...
        <div className="flex-1 overflow-y-auto py-2">
          {(!sidebarCollapsed || (isMobileView && showMobileSidebar)) && (
            <div className="px-2 mb-2">
              <h3 className="text-xs font-medium text-slate-500 dark:text-slate-400 px-2">Recent</h3>
            </div>
          )}
          <div className="space-y-1 px-2">
            {recentChats.map((chat) => (
              <Button
                ...
                className={cn(
                  "w-full justify-start gap-2 text-slate-800 dark:text-slate-200",
                  activeChat === chat.id && "bg-slate-100 dark:bg-slate-700",
                  sidebarCollapsed && "justify-center px-0"
                )}
              >
          ...
          {sidebarCollapsed && !isMobileView ? (
            <div className="flex flex-col items-center space-y-2">
              <Button
                ...
                className="w-8 h-8 p-0 text-slate-500 hover:text-foreground hover:bg-neutral-100 dark:hover:bg-neutral-800"
              >
            ...
            <div className="flex items-center justify-between">
              <Button
                ...
                size="sm"
                onClick={() => setDarkMode(!darkMode)}
                className="text-slate-500 hover:text-foreground hover:bg-slate-100 dark:hover:bg-slate-800"
              >
      ...
      <div className="flex-1 flex flex-col min-w-0 main-content-bg">
        {/* Top Bar */}
        <header className="h-16 flex items-center justify-between dark:bg-neutral-900 border-b border-neutral-100 dark:border-neutral-900 px-4 md:px-6">
          {isMobileView && (
            <Button
              variant="ghost"
              size="sm"
          ...
          <div className="flex items-center gap-4">
            <h1 className="font-medium text-lg text-neutral-100 truncate">{getCurrentChatTitle()}</h1>
          </div>

          <div className="flex items-center gap-2 md:gap-3">
            <Button size="sm" className="bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm rounded-lg px-3 md:px-4 h-8 text-xs md:text-sm">
              Get Pro
            </Button>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="ghost"
                  className="w-9 h-9 p-0 text-foreground hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full"
                >
              ...
              >
                <DropdownMenuItem className="text-foreground hover:bg-slate-100 dark:hover:bg-slate-700">
        ...
        <div className="flex-1 flex flex-col min-h-0">
          {!hasStartedChat ? (
            ...
            <div className="flex-1 flex items-center justify-center p-4 md:p-8">
              <div className="max-w-2xl w-full space-y-6 md:space-y-8 text-center">
                ...
                <div className="space-y-4 md:space-y-6">
                  <h1 className="text-2xl md:text-4xl font-semibold tracking-tight text-neutral-100 riik-text">
                ...
                <div className="relative">
                  <div className="relative bg-white dark:bg-neutral-600 rounded-xl md:rounded-3xl shadow-lg border border-slate-200 dark:border-neutral-700">
                    <Input
                      ref={inputRef}
                      value={input}
                      ...
                      className="w-full h-12 md:h-16 text-sm md:text-base pl-4 md:pl-6 pr-12 md:pr-16 rounded-xl md:rounded-3xl border-0 bg-transparent focus:ring-0 focus:outline-none text-neutral-100 placeholder:text-neutral-400"
                    />
                    <Button
            ...
            <>
                      ...
                      <div className={cn("flex gap-4", message.role === "user" ? "justify-end" : "justify-start")}>
                        ...
                        <div className={cn("max-w-2xl", message.role === "user" ? "ml-12" : "mr-12")}>
                          ...
                          {message.role === "assistant" && message.thoughtContent && (
                            <div className="mb-4">
                              <Collapsible>
                                <CollapsibleTrigger className="flex items-center gap-2 text-sm py-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded px-2 -mx-2 transition-colors">
                                  <div className="w-1.5 h-1.5 bg-purple-500 rounded-full"></div>
                          ...
                          {/* Message Content */}
                          <div className={cn(message.role === "user" ? "user-message" : "assistant-message", "text-sm md:text-base")}>
              ...
              {/* Fixed Input */}
              <div className="bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 p-3 md:p-4 sticky bottom-0 left-0 right-0">
                <div className="max-w-4xl mx-auto relative">
                  <div className="relative bg-white dark:bg-slate-800 rounded-xl md:rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700">
                    <Input
                      ref={inputRef}
                      value={input}
                      onChange={(e) => setInput(e.target.value)}
                      ...
                      className="w-full h-10 md:h-12 pl-3 md:pl-4 pr-10 md:pr-12 rounded-xl md:rounded-2xl border-0 bg-transparent focus:ring-0 focus:outline-none text-foreground placeholder:text-slate-400 text-sm md:text-base"
                    />
                    <Button
                      onClick={() => handleSendMessage()}
                      disabled={!input.trim() || aiState !== "idle"}
                      size="sm"
                      className="absolute right-1.5 md:right-2 top-1.5 md:top-2 h-7 md:h-8 w-7 md:w-8 p-0 bg-slate-400 hover:bg-slate-500 dark:bg-slate-600 dark:hover:bg-slate-500 text-white rounded-full"
                    >
